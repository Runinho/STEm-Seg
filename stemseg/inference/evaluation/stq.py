...
def _update_dict_stats(stat_dict: MutableMapping[int, torch.Tensor],
                       id_array: torch.Tensor, mask: torch.Tensor):
  """Updates a given dict with corresponding counts."""
  # for each time frame
  for t, (a, m) in enumerate(zip(id_array, mask)):
    ids, counts = a[m].unique(return_counts=True)
    for idx, count in zip(ids.numpy(), counts):
      if idx in stat_dict:
        stat_dict[idx][t] = count.item()
      else:
        stat_dict[idx] = {t: count.item()}
...
class STQuality(object):
  ...
  def update_state(self, y_true: torch.Tensor, y_pred: torch.Tensor,
    ...
    if self._ignore_label > self._num_classes:
      ...
      #     self._num_classes)
      semantic_label[semantic_label == self._ignore_label] = self._num_classes
      semantic_prediction[semantic_prediction == self._ignore_label] = self._num_classes
    ...
    if sequence_id in self._iou_confusion_matrix_per_sequence:
      self._iou_confusion_matrix_per_sequence[sequence_id] += (
          confusion_matrix(
              semantic_label.reshape(-1),
              semantic_prediction.reshape(-1),
              labels = np.arange(self._confusion_matrix_size)
          ),
      )
    ...
    # we take the union
    non_crowd_intersection = torch.logical_and(label_mask, prediction_mask)
    # TODO: WHY ARE WE "BIT-PACKING" THEM INTO ONE INT?????
    assert y_pred.max() < self._offset, "can not pack into one int32 (y_pred larger than offset)"
    assert y_true.max() < self._offset, "can not pack into one int32 (y_true larger than offset)"
    assert y_true.max() * self._offset < np.iinfo(np.int32).max, "does not fit into int32"
    intersection_ids = (
        y_true * self._offset +
        y_pred)
    _update_dict_stats(seq_intersects, intersection_ids, non_crowd_intersection)
  ...
  def result(self) -> Dict[Text, Any]:
    ...
    for index, confusion in enumerate(
      ...
      _include_indices = self._include_indices[:len(confusion)]
      removal_matrix[_include_indices, :] = 1.0
